        ...
        <div class="container-fluid padding-side">
            <div class="row g-5">
                <main class="service-grid col-md-9">
                    ...
                    @endforeach           
                    @endif
                    <form action="{{route('modelUpdate', ['id'=>$dress->id])}}" class="form-group flex-wrap p-4 border rounded-4" method="POST" enctype="multipart/form-data">
                        @method('PUT')
                        @csrf
                        <h2 class=" fs-2  my-3 mb-5">Model New</h2>
                        <div class="col-lg-12 my-4">
                          <label for="na,e" class="form-label ">Name</label>
                          <input type="text" id="name" name="name" class="form-control ps-3" value="{{ $dress->name }}">
                        </div>
                        <div class="col-lg-12 my-4">
                          <label for="slug" class="form-label ">Slug</label>
                          <input type="text" id="slug" name="slug" class="form-control ps-3" value="{{ $dress->slug }}">
                        </div>
                        <div class="row">
                            <div class="col-lg-6 my-4">
                                <label for="slug" class="form-label ">Price</label>
                                <input type="Number" id="price" name="price" class="form-control ps-3" value="{{ $dress->price }}">
                            </div>
                            <div class="col-lg-6 my-4">
                                <label for="slug" class="form-label ">Size</label>
                                <input type="text" id="size" name="size" class="form-control ps-3" value="{{ $dress->size }}">
                            </div>
                        </div>
                        <div class="col-lg-12 my-4">
                        ...
                        <div class="col-lg-3 my-4">
                            ...
                            <div class="image-upload-container">
                                <div class="image-preview" id="imagePreview">
                                    <img src="{{ $dress->image ? asset($dress->image) :  url('/assets/img/default2.jpg') }}" alt="Preview" class="image-preview__image" style="{{ $dress->image ? 'display:block;' : 'display:none;' }}">
                                        
                                    <span class="image-preview__default-text" style="{{ $dress->image ? 'display:none;' : 'display:block;' }}">
                                        No image selected
                                    </span>
                                </div>
